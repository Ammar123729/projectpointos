                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="card-body">
                                        ...
                                        @endif
                                        <form action="{{ route('purchase.retrun.item') }}" method="POST">
                                            @csrf
                                            ...
                                            <div id="item-rows">
                                                ...
                                                <div class="item-row">
                                                    ...
                                                    <!-- Return Quantity -->
                                                    <label for="purchase_quantity">Return Quantity:</label>
                                                    <input type="number" name="purchase_quantity[]" class="purchase_quantity form-control" oninput="calculateTotal()" required>

                                                    <!-- Return Payment -->
                                                    <label for="return_payment">Return Payment:</label>
                                                    <input type="number" name="return_payment[]" class="return_payment form-control" oninput="calculateTotal()" required>

                                                    <!-- Total Value for individual item -->
                                            ...
                                            <div class="mt-3">
                                                <label for="totaldate">Combined Total:</label>
                                                <input type="date" id="totaldate" name="date" class="form-control">
                                            </div>
                                            <!-- Display combined total for all items -->
                                            <div class="mt-3">
                                                <label for="total_payment">Combined Total:</label>
...
<script>
    ...
    //         // Add this row's total to the combined total
    //         combinedTotal += totalValue;
    //     });

    //     // Display the combined total in the designated field
    //     document.getElementById('combined_total').value = combinedTotal.toFixed(2);
    // }
    ...
    function calculateTotal() {
        ...
        // Iterate over each item row to calculate total
        document.querySelectorAll('.item-row').forEach(function(row) {
            const returnQuantity = row.querySelector('.purchase_quantity').value || 0;
            const returnPayment = row.querySelector('.return_payment').value || 0;
            const totalValueField = row.querySelector('.total_value');

            // Calculate total value for this row and display it
            const totalValue = returnQuantity * returnPayment;
            totalValueField.value = totalValue.toFixed(2); // Show two decimal places

            // Accumulate the total value for all items
            totalPayment += parseFloat(totalValue);
        });

        // Display the combined total value for all items
